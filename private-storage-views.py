...
class PrivateStorageView(View):
    ...
    def serve_file(self, private_file):
        ...
        type = mime.from_file(settings.PRIVATE_STORAGE_AUX+private_file.request.path)
        with open(settings.PRIVATE_STORAGE_AUX+private_file.request.path, 'rb') as f:
            data = f.read()
            fernet = Fernet(settings.SECRET_KEY)
            decrypted = fernet.decrypt(data)
            response = HttpResponse(decrypted, content_type=type)
            #response.write(decrypted)(escribe otra vez en archivo creado)
            return response
